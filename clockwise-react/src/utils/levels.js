import { generateHandsForTime, formatTime } from './clock';
...
const randomFromExcluding = (array, exclude) => {
...
const convertTo24Hour = (hour12, minute, isPM) => {
    let hour24;
    if (hour12 === 12) {
        hour24 = isPM ? 12 : 0;
    } else {
...
export const format24HourTime = (hour, minute) => {
    return `${hour.toString().padStart(2, '0')}:${minute.toString().padStart(2, '0')}`;
};

export const generateQuestion = (levelConfig) => {
    let hour = randomFromExcluding(levelConfig.hourOptions, lastHour);
    let minute;
    
    if (levelConfig.minuteOptions === 'any') {
        do {
            minute = Math.floor(Math.random() * 60);
        } while (minute === lastMinute && levelConfig.minuteOptions === 'any');
    } else {
        minute = randomFromExcluding(levelConfig.minuteOptions, lastMinute);
    }
    ...
    if (levelConfig.questionType === 'elapsed' || levelConfig.questionType === 'calculation' || 
        ...
        elapsedMinutes = randomFrom([15, 20, 30, 45]);
    }
    
    const includeMinute = levelConfig.hands.includes('minute');
    const hands = generateHandsForTime(hour, minute, second, {
    ...
    if (levelConfig.show24Hour) {
        const time24 = convertTo24Hour(hour, minute, isPM);
...
export const generateChoices = (correctAnswer, levelConfig) => {
    ...
    while (choices.length < 4) {
        ...
        if (variation < 0.33) {
            ...
            if (levelConfig.minuteOptions === 'any') {
                wrongMinute = (correctAnswer.minute + randomFrom([5, 10, 15, -5, -10, -15]) + 60) % 60;
            } else {
                const options = levelConfig.minuteOptions.filter(m => m !== correctAnswer.minute);
                wrongMinute = options.length > 0 ? randomFrom(options) : (correctAnswer.minute + 15) % 60;
            }
        } else if (variation < 0.66) {
            wrongHour = randomFrom(levelConfig.hourOptions.filter(h => h !== correctAnswer.hour));
            if (!wrongHour) wrongHour = (correctAnswer.hour % 12) + 1;
            wrongMinute = correctAnswer.minute;
        } else {
            wrongHour = randomFrom(levelConfig.hourOptions);
            if (levelConfig.minuteOptions === 'any') {
                wrongMinute = Math.floor(Math.random() * 60);
            } else {
                wrongMinute = randomFrom(levelConfig.minuteOptions);
            }
        }
...
export const getHint = (levelConfig, hintIndex = 0) => {
...
export const getMediatedPrompt = (levelConfig) => {
    ...
    return randomFrom(prompts) || 'Great work! Keep practicing!';
};
...
export const getExplanation = (question, userAnswer, isCorrect, levelConfig = null) => {
    ...
    if (isCorrect) {
        ...
        return `The hour hand is at ${correct.hour}, minute hand shows ${correct.minute} minutes. That's ${formatTime(correct.hour, correct.minute)}!`;
    } else {
        if (correct.minute === 0) {
        ...
        return `The minute hand at ${minutePosition} means ${correct.minute} minutes. The hour hand between ${correct.hour} and ${(correct.hour % 12) + 1} means it's still ${correct.hour}. The correct time is ${formatTime(correct.hour, correct.minute)}.`;
    }
};
