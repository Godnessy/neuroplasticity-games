import { getWeakAreas } from './storage';
import { getLevel } from './levels';

export const calculateSessionAccuracy = (session) => {
...
export const shouldAdvanceLevel = (session) => {
...
export const shouldReduceDifficulty = (session) => {
    if (!session || session.answers.length < 5) {
        return false;
    }
    const accuracy = calculateSessionAccuracy(session);
...
export const getRecommendedAction = (session, currentLevel) => {
    const accuracy = calculateSessionAccuracy(session);
    ...
    if (accuracy < 0.60 && session.answers.length >= 5) {
        return {
            ...
            adjustments: getScaffolds(currentLevel, accuracy)
        };
    }
...
export const getScaffolds = (level, accuracy) => {
    const scaffolds = [];
...
export const adjustTimeLimit = (baseTime, recentAnswers) => {
...
export const getPerformanceTrend = (sessions) => {
    if (!sessions || sessions.length < 2) {
        return 'insufficient_data';
    }
    
    const recent = sessions.slice(-5);
    const accuracies = recent.map(s => calculateSessionAccuracy(s));
...
export const calculateOptimalDifficulty = (progress) => {
...
export const generateProgressReport = (progress, sessions) => {
    const report = {
        ...
        trend: getPerformanceTrend(sessions),
        levelBreakdown: {},
        areasForImprovement: [],
        strengths: []
    };
...
export const estimateTimeToCompletion = (progress) => {
    const currentLevel = progress.currentLevel || 1;
